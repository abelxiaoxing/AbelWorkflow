...
async function runFullInit(options) {
  ...
  if (await promptConfirm("是否填写 grok-search 环境变量？", false)) {
    await configureGrokSearchEnv(options.agentsDir);
  }
  if (await promptConfirm("是否填写 context7-auto-research 环境变量？", false)) {
    await configureContext7Env(options.agentsDir);
  }
  if (await promptConfirm("是否填写 prompt-enhancer 环境变量？", false)) {
