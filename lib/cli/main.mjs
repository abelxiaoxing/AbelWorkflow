...
async function configureClaudeForPaths(paths) {
  const metadata = await readInstallMetadata(paths);
  const packageVersion = await packageVersionFor(paths, metadata);
  const permissionProfile = await selectOrCancel(getClaudePermissionProfilePromptOptions(
...
async function runInteractiveMenu(options) {
  ...
  const cliToolMenus = {
    "pi-cli": {
      ...
      actions: {
        ...
        "pi-api": async () => configurePiApi(options.paths, ensurePiResourcesLinked, promptApi)
      }
    },
    "codex-cli": {
      tool: "codex",
  ...
  const menuActions = {
    ...
    "grok-search": async () => configureGrokSearchEnv(options.paths, ensureWorkflowPresent, promptApi),
    context7: async () => configureContext7Env(options.paths, ensureWorkflowPresent, promptApi),
