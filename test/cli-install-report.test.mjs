...
test("CLI forwards canonical Paths and nested feature overrides", () => {
  const source = readFileSync(new URL("../lib/cli/main.mjs", import.meta.url), "utf8");
  ...
  assert.match(source, /featureOverrides:\s*\{\s*augmentContextEngine\s*\}/u);
});
...
test("direct provider configuration finalizes install metadata through schema v2", () => {
  ...
  assert.doesNotMatch(
    source,
    /writeInstallMetadata\(paths,\s*\{\s*\.\.\.metadata,\s*(?:managedCodexAuthKeys|claudePermissionProfile):/u
  );
});
