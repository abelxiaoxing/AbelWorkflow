...
首次升级只迁移 v1 metadata 的 `linkedTargets`，再写入 metadata v2。`managedChildren` 不迁移为 asset 所有权，也不会触发现场认领。v2 使用内容 hash 记录精确托管路径：
...
配置默认写入 `bypassPermissions` 全权限模式（YOLO），第三方 API 仅使用 API Key 认证，且不再添加任何 MCP 权限；现有 permissions、deny、hooks 和 timeout 保持不变。
...
- Prompt Enhancer 移除第三方 API/Python 运行时与环境变量配置，原有运行时配置不再生效；现在由当前 agent 按模板直接改写提示词，无需额外配置。
...
- Claude 配置包含 `bypassPermissions` 默认权限模式。
